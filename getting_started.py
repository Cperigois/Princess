...
astromodel1 = AstroModel(name='BBH_YSCs', original_path=path_2_cat_folder + Cat, flags=Flags,
                         sep="\t")
...
In this part is detailed the context of the study starting by defining the range of frequency Freq and the waveforms 
WF_approx to use. In this version the range has to be linear in the future specific function will be added to allow log 
scale, in particular for LISA band. The available waveforms are the ones define in PyCBC and the analytic one from 
Ajith2011. The calculation with Ajith waveforms is computationnalymore expensive and therefore not recommended.

'''

# Frequency range for the calculation
# Freq = np.linspace(5, 250, 244)
frequency_array = np.arange(240) + 10

# Waveform you would like to use from Pycbc, write Ajith for analytical ones (longer computation)
WF_approx = "IMRPhenomD"

'''
In Princess two classes has been build for this purpose in the file Starter/Detection in order to define Detectors and 
combine them to build a Network. The Detector class takes in entry several parameters:

    det_name: (str) is the name you give to the detector
    configuration: (str) is the preset configuration(location and orientation) of the detector.  
    origin: (str) True if the sensitivity is available in PyCBC, else False
...
H = Detection.Detector(name='H', configuration='H', origin='Pycbc', psd_file='aLIGODesignSensitivityP1200087', freq=frequency_array)
L = Detection.Detector(name='L', configuration='L', origin='Pycbc', psd_file='aLIGODesignSensitivityP1200087', freq=frequency_array)
V = Detection.Detector(name='V', configuration='V', origin='Pycbc', psd_file='AdVDesignSensitivityP1200087', freq=frequency_array)
...
The second class Network allow to combine different detectors to build a Network.A Network takes in entry:
    ...
    efficiency: (float) between 0 and 1 define the effective time of observation of the Network. For example during O3a,
        in the Hanford-Livinstone-Virgo network only 50% of the data can be used with the three pipelines. The rest of 
        the time at least on detector pipeline was unusuable.
    SNR_thrs: (int or float) Define the SNR threshold for which we assume a source is detectable.
    SNR_sub: (int or float) Define the SNR threshold to substract the sources. For example its commonly assumed that in 
        HLV all source with an SNR above 8 are resolved. However for a reason of parameter uncertainty the calculation 
        of the residual background is done by subtracting only source with a SNR above 12.
...
HLV = Detection.Network(name='HLV', compo=[H, L, V], pic_file='AuxiliaryFiles/PICs/Design_HLV_flow_10.txt',
                        efficiency=0.5, SNR_thrs=12)

Networks = [HLV]

'''Finally the method compute_SNR, compute the SNR of each sources of the model, and update the catalogue(s) with a new 
parameter column named by net_name containing the SNR in the corresponding network.'''

astromodel1.compute_SNR_opt(Networks, frequency_array, approx=WF_approx)
...
Prepare the calculation of the background with the class Princess :
    ...
    freq: (np.array) frequency range, preferentially the one used since the beginning. In linear scale before the LISA 
        update
    approx: (str) waveform approximation
    freq_ref: (list of float) list of frequency of interest for the study, is usually 10Hz for 3G detectors and 25Hz for 
        2G.
...
This part of the code aims to extract reference values for the predicted background. Usual values are the amplitude at 
    10 and 25 Hz, the SNR, the number of resolved sources, the ratio of detected sources, and the ratio between residuals and total backgrouns at a reference value.
